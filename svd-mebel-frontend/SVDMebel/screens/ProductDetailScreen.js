...
const ProductDetailScreen = ({ route }) => {
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      try {
        ...
        const res = await axios.get(`http://192.168.59.67:5000/products/product/${productId}`, {
    ...
    const checkProductInCart = async () => {
      ...
      try {
        ...
        const cartRes = await axios.get(`http://192.168.59.67:5000/cart/${userId}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const renderImageItem = ({ item }) => (
    <View style={styles.sliderItem}>
      <Image
        source={{ uri: `http://192.168.59.67:5000${item}` }}
  ...
  const handleArView = () => {
    if (product?.ar_model_path) {
      const arUrl = `https://192.168.59.67:443/web/ar.html?model=${encodeURIComponent(product.ar_model_path)}`;
