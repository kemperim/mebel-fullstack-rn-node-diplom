...
const CartScreen = ({ navigation }) => {
  ...
  const fetchCart = async () => {
    ...
    try {
      ...
      const response = await axios.get(`http://192.168.66.67:5000/cart/${userId}`, {
  ...
  const updateQuantity = async (itemId, newQuantity) => {
    ...
    try {
      ...
      await axios.put(`http://192.168.66.67:5000/cart/update/${itemId}`, {
  ...
  const removeItem = async (itemId) => {
    try {
      ...
      await axios.delete(`http://192.168.66.67:5000/cart/remove/${itemId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchCart();
    } catch (error) {
    ...
    return (
      <Animated.View style={styles.cartItem}>
        <View style={styles.imageContainer}>
          <Image 
            source={{ uri: `http://192.168.66.67:5000${item.Product.image}` }} 
