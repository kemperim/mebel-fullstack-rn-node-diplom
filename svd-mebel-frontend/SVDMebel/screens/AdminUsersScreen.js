...
const AdminUsersScreen = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await axios.get("http://192.168.66.67:5000/admin/users", {
  ...
  const updateUser = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://192.168.66.67:5000/admin/users/${selectedUser.id}`,
        {
  ...
  const toggleBlockUser = async () => {
    ...
    try {
      const token = await AsyncStorage.getItem("token");
      await axios.put(
        `http://192.168.66.67:5000/admin/users/${selectedUser.id}/block`,
    ...
    Alert.alert("Удаление", "Вы уверены, что хотите удалить пользователя?", [
      { text: "Отмена", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            const token = await AsyncStorage.getItem("token");
            await axios.delete(`http://192.168.66.67:5000/admin/users/${selectedUser.id}`, {
              headers: { Authorization: `Bearer ${token}` },
            });
