...
const EditProduct = () => {
    ...
    useEffect(() => {
        const fetchProductData = async () => {
            try {
                ...
                const productResponse = await axios.get(`http://192.168.230.67:5000/products/product/${productId}`);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await axios.put(`http://192.168.230.67:5000/products/edit/${productId}`, formData, {
                ...
                <ScrollView horizontal style={styles.imageScrollView} showsHorizontalScrollIndicator={false}>
                    {images.map((image, index) => (
                        <View key={index} style={styles.imageContainer}>
                            <Image
                                source={{ uri: `http://192.168.230.67:5000${image}` }}
                                style={styles.image}
                            />
                            <TouchableOpacity
