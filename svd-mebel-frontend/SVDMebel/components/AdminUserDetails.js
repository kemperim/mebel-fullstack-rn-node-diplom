...
const AdminUserDetailsScreen = ({ route, navigation }) => {
  ...
  const fetchUserDetails = async () => { // Убираем userId из параметров
    try {
      ...
      const response = await axios.get(`http://192.168.230.67:5000/admin/users/${userId}`, {
  ...
  const updateUser = async () => {
    try {
      const token = await AsyncStorage.getItem("token");
      await axios.put(
        `http://192.168.230.67:5000/admin/users/${userId}`,
    ...
    Alert.alert("Удаление", "Вы уверены, что хотите удалить пользователя?", [
      { text: "Отмена", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            const token = await AsyncStorage.getItem("token");
            await axios.delete(`http://192.168.230.67:5000/admin/users/${userId}`, {
              headers: { Authorization: `Bearer ${token}` },
            });
            navigation.goBack();
          } catch (error) {
